<img src="{{ asset('image/departments/' . $department->img) }}" alt="{{ $department->name }}" class="hoverZoomLink"
    id="img">
...
@foreach ($interesteds as $interested)
    <button class="removeall_btn btn btn-dark"
        name="deleteall"style="margin-top: -165px;margin-left: 600px;">RemoveAll</button>
@break
...
@if ($users->role == $check)Gender : Admin
@else
    Gender : Customer @endif
</h3>
{{ $numfav }}
{{ $numorders }}
...
<div style="margin-left: 200px;margin-top: -35px">
    <button orders_id={{ $order->id }} name="favourite" class="AddFav btn btn-lg"><i class="fa fa-heart"
            id="fav{{ $order->id }}" style="color: gold;"></i></button>
    @isset($interesteds)
        @foreach ($interesteds as $interested)
...
@foreach ($orders_user as $oruser)
    @if ($oruser->id == $order->id)
        ...
        <button order_id={{ $order->id }} class="delete_btn btn btn-danger"
            style="margin-top: -200px;margin-left: 400px;">DELETE</button>
    @else
    @endif
@endforeach
@endisset
